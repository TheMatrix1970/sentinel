...
def has_quaz_conf():
    ...
    valid_quaz_conf = False

    # ensure quaz_conf exists & readable
    #
    # if not, print a message stating that Quaz Core must be installed and
    # configured, including JSONRPC access in quaz.conf
    try:
        f = io.open(config.quaz_conf)
        valid_quaz_conf = True
    except IOError as e:
        print(e)

    return valid_quaz_conf
...
def main():
    ...
    if not has_quaz_conf():
        print("QuazCore must be installed and configured, including JSONRPC access in quaz.conf")
        sys.exit(1)
