...
def is_valid_quaz_address(address, network='mainnet'):
    ...
    quaz_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Quaz address
        return False

    if (address_version != quaz_version):
        return False
...
# shims 'til we can fix the quazd side
def SHIM_serialise_for_quazd(sentinel_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = COIN2FLYD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in quazd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # quazd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    quazd_hex = serialise(obj)
    return quazd_hex


# shims 'til we can fix the quazd side
def SHIM_deserialise_from_quazd(quazd_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(quazd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('quaz.conf')
