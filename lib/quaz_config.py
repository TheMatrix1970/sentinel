...
class QuazConfig():
    ...
    def slurp_config_file(self, filename):
        # read quaz.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is quaz.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from quaz.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value.strip() for (key, value) in match}

        # standard Quaz defaults...
        default_port = 20099 if (network == 'mainnet') else 21099

        # use default port for network if not specified in quaz.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from quaz.conf
