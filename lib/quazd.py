...
class QuazDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some quazd methods
    ...
    @classmethod
    def from_quaz_conf(self, quaz_dot_conf):
        from quaz_config import QuazConfig
        config_text = QuazConfig.slurp_config_file(quaz_dot_conf)
        creds = QuazConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = quazlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = quazlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = quazlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_quazd_proto_version_with_sentinel_ping)
