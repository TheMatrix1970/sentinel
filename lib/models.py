...
class GovernanceObject(BaseModel):
    ...
    # sync quazd gobject list with our local relational DB backend
    @classmethod
    def sync(self, quazd):
        golist = quazd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_quazd(quazd, item)
    ...
    @classmethod
    def import_gobject_from_quazd(self, quazd, rec):
        import decimal
        import quazlib
        import inflection
        ...
        # shim/quazd conversion
        object_hex = quazlib.SHIM_deserialise_from_quazd(object_hex)
        objects = quazlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from quazd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from quazd, with every run
        ...
        # get/create, then sync payment amounts, etc. from quazd - Quazd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(quazd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from quazd! %s" % e)
            govobj.vote_delete(quazd)
            return (govobj, None)
    ...
    def vote_delete(self, quazd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(quazd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, quazd, signal, outcome):
        import quazlib
        ...
        output = quazd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = quazlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(quazd, signal)

    def sync_network_vote(self, quazd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = quazd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = COIN2FLYD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 quaz addr, non-multisig
            if not quazlib.is_valid_quaz_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Quaz address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (quazlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/QuazDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import quazlib
        obj_data = quazlib.SHIM_serialise_for_quazd(self.serialise())
    ...
    def prepare(self, quazd):
        try:
            object_hash = quazd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not quazlib.is_valid_quaz_address(addr, config.network):
    ...
    def hash(self):
        import quazlib
        return quazlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = COIN2FLYD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, quazd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - quazd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, quazd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - quazd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, quazd):
        now = int(time.time())
        return (self.created_at < (now - quazd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, quazd):
        if self.is_expired(quazd):
            return False

        return True

    def is_deletable(self, quazd):
        if self.is_expired(quazd):
            return True
